...
class _FocusModeScreenState extends State<FocusModeScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      // 判断是否在顶部
      setState(() {
        _isAtTop = _scrollController.position.pixels == 0.0;
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: _buildFocusModeContainer(sortedTasks),
  ...
  Color _getBackgroundCOlor(Task task) {
    ...
    return task.isImportant ? color : color.withOpacity(0.8);
  }
  ...
  String _formatDuration(Duration duration) {
    ...
    return '$days $dayCorrective, $hours $hourCorrective, $minutes $minuteCorrective $secs $secsCorrective is remaining';
  }
